...
type Intent = {
  ...
  table: string;
  customerNote: string;
  // Optional gratuity (2026-09-25). Never part of the Square ORDER: it rides on
  // the card payment as tip_money, so it lands in Square's tip reporting and is
...
// Percent tips are figured on the ticket subtotal after any promo, before tax
// and fee — the same base the page uses, so the number shown is the number charged.
function tipFor(body: Record<string, unknown>, base: number): { tipCents: number; tipPercent: number } {
  const pct = Math.floor(Number(body.tipPercent ?? body.tip_percent ?? 0) || 0);
  if (pct && TIP_PERCENTS.includes(pct)) {
    return { tipCents: Math.round((Math.max(0, base) * pct) / 100), tipPercent: pct };
  }
  const flat = Math.floor(Number(body.tipCents ?? body.tip_cents ?? 0) || 0);
  return { tipCents: Math.max(0, Math.min(MAX_TIP_CENTS, flat)), tipPercent: 0 };
}
...
async function quote(req: Request, body: Record<string, unknown>) {
  ...
  return {
    ...
    grand_total_cents: totals.total_cents + intent.tipCents,
    currency: CURRENCY,
  };
}
...
function confirmationHtml(
  ...
  const tip = intent.tipCents ?? 0;
  const cardPart = totals.total_cents - giftCents + tip;
  const ev = intent.event;
  ...
  return `
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;color:#06273a">
      ...
      <table style="width:100%;border-top:1px solid #ddd;margin-top:14px">
        ...
        ${tip ? row("Tip" + (intent.tipPercent ? ` (${intent.tipPercent}%)` : ""), usd(tip)) : ""}
        ${row("<b>Total</b>", "<b>" + usd(totals.total_cents + tip) + "</b>")}
...
function paidResult(pmt: Record<string, any>) {
  return {
    ...
    tip_cents: pmt.tip_cents ?? 0,
    grand_total_cents: (pmt.total_cents ?? 0) + (pmt.tip_cents ?? 0),
    card_cents: pmt.gift_cents ? (pmt.card_cents ?? 0) : (pmt.total_cents ?? 0) + (pmt.tip_cents ?? 0),
...
async function payWithGift(o: {
  ...
  email: string; note: string; giftMax?: number; tipCents?: number;
  ...
  const tip = Math.max(0, o.tipCents ?? 0);
  // A tip goes on the credit card, never the gift card. A gift card that covers
  // the whole order leaves no card payment to carry it.
  if (tip > 0 && rest <= 0) {
    throw new PublicError(
      "Tips can only go on a credit card, and your gift card covers the whole order. " +
      "Remove the tip, or use a smaller gift card amount. Nothing was charged.",
    );
  }
  ...
  if (rest > 0) {
    ...
    const c = await square("/v2/payments", {
      ...
      body: JSON.stringify({
        ...
        tip_money: tip > 0 ? money(tip) : undefined,
        autocomplete: false,
        reference_id: o.code,
        buyer_email_address: o.email,
        note: o.note,
...
async function pay(req: Request, body: Record<string, unknown>) {
  ...
  // Same idea for the tip: a percent tip can only move if the discount moved,
  // and that is refused above — but never charge a bigger tip than was shown.
  const expectedTip = body.expectedTipCents ?? body.expected_tip_cents;
  if (expectedTip != null && intent.tipCents > cents(expectedTip)) {
    throw new PublicError("The tip changed while you were checking out. Nothing was charged — please review and try again.");
  }
  ...
  try {
    ...
    const res = await square("/v2/payments", {
      ...
      body: JSON.stringify({
        ...
        tip_money: intent.tipCents > 0 ? money(intent.tipCents) : undefined,
        autocomplete: true,
        reference_id: code,
...
async function status(body: Record<string, unknown>) {
  ...
  const q = admin
    ...
    .select("id,status,total_cents,subtotal_cents,discount_cents,tax_cents,fee_cents," +
      ...
      "gift_cents,card_cents,tip_cents");
...
async function refund(body: Record<string, unknown>) {
  ...
  // The tip rode on the card payment, so it is refundable along with the order.
  const total = cents(row.total_cents) + cents(row.tip_cents ?? 0);
